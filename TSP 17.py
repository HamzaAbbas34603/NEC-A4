...
def calculate_total_distance(route, problem):
    ...
    for i in range(len(route)):
        start_node = route[i]
        end_node = route[(i + 1) % len(route)]
        
        # Find the corresponding Weight in the matrix
        total_distance += problem.get_weight(start_node - 1, end_node - 1)  # Adjust for 0-based indices
